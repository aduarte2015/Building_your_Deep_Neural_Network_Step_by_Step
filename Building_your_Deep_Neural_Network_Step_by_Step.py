...
def initialize_parameters(n_x, n_h, n_y):
    ...
    # Initialize weights with random values and scale them by 0.01
...
def initialize_parameters_deep(layer_dims):
    ...
    for l in range(1, L):
        ...
        # Ensure that the shapes of the weights and biases are correct
...
def linear_backward(dZ, cache):
    ...
    dW = (1 / m) * np.dot(dZ, A_prev.T)  # Gradient with respect to W
    db = (1 / m) * np.sum(dZ, axis=1, keepdims=True)  # Gradient with respect to b
    dA_prev = np.dot(W.T, dZ)  # Gradient with respect to A_prev
...
def linear_activation_backward(dA, cache, activation):
    ...
    if activation == "relu":
        ...
        Z = activation_cache
        dZ = dA * (Z > 0)  # dZ = dA * ReLU'(Z), where ReLU'(Z) is 1 for Z > 0 and 0 otherwise
        ...
        dZ = dA * A * (1 - A)  # dZ = dA * sigmoid'(Z), where sigmoid'(Z) = sigmoid(Z) * (1 - sigmoid(Z))
...
def L_model_backward(AL, Y, caches):
    ...
    dAL = AL - Y  # Compute the initial gradient for the output layer
    
    # Lth layer (SIGMOID -> LINEAR) gradients
    current_cache = caches[L-1]
    linear_cache, activation_cache = current_cache
    dZL = dAL * sigmoid_derivative(activation_cache)  # Derivative of sigmoid
    dA_prev_temp, dW_temp, db_temp = linear_backward(dZL, linear_cache)
    ...
    for l in reversed(range(L-1)):
        ...
        linear_cache, activation_cache = current_cache
        dZ = grads["dA" + str(l + 1)] * relu_derivative(activation_cache)  # Derivative of ReLU
...
def update_parameters(params, grads, learning_rate):
    ...
    parameters -- python dictionary containing your updated parameters 
                  ...
                  parameters["b" + str(l)] = ...
    """
    parameters = copy.deepcopy(params)  # Deepcopy to preserve original params
    L = len(parameters) // 2  # Number of layers in the network (since we have W and b for each layer)
